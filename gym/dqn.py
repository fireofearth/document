...
class DQNAgent:
    def __init__(self):
        ...
        # target model
        # where we call .predict() against every step
    ...
    def get_qs(self, state):
        input = np.array(state).reshape(-1, *state.shape) / 255
    ...
    def train(self, terminal_state, step):
        ...
        X = np.array(X) / 255
        y = np.array(y)
        self.model.fit(X, y, batch_size=MINIBATCH_SIZE,
...
for episode in tqdm(range(1, EPISODES+1), unit='episode'):
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        #         epsilon=epsilon)
        tf.summary.scalar('reward_avg', average_reward, step=episode)
        tf.summary.scalar('reward_min', min_reward, step=episode)
        tf.summary.scalar('reward_max', max_reward, step=episode)
